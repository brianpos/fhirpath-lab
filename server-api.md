...
##### AST definition
The AST is a JSON representation of the Abstract Syntax Tree for the expression, which is useful for debugging. The structure is roughly based on the Firely AST, but simplified to only include the key information needed for debugging, then further expanded with the needs of the fhirpath-lab's interface.
The AST can optionally include type information (if available) and position information (if available).
...
```
*The source also includes an `id` property, however that is generated by the fhirpath-lab and used internally.*

**Field Descriptions:**

| Field | Type | Cardinality | Description |
|-------|------|:-----------:|-------------|
| `ExpressionType` | string | 1..1 | The type of AST node (e.g., "FunctionCallExpression", "ConstantExpression", "ChildExpression", "VariableRefExpression") |
| `Name` | string | 1..1 | The name or value associated with the node (function name, constant value, property name, etc.) |
| `Arguments` | JsonNode[] | 0..* | Child nodes in the AST tree structure |
| `ReturnType` | string | 0..1 | Optional static type information if available from the engine's type system |
| `Position` | number | 0..1 | Character offset position in the original expression where this node starts (0-based) |
| `Length` | number | 0..1 | Length in characters of this node in the original expression |
| `Line` | number | 0..1 | Line number in the original expression (1-based, alternative to Position) |
| `Column` | number | 0..1 | Column number in the original expression (1-based, used with Line) |
| `SpecUrl` | string | 0..1 | URL to FHIRPath specification section for this node (populated by the lab UI) |

**Common ExpressionType Values:**
- `ConstantExpression`: Literal values (strings, numbers, booleans, dates)
- `FunctionCallExpression`: Function calls (e.g., `where()`, `select()`, `first()`)
- `ChildExpression`: Property navigation (e.g., `Patient.name`, `address.city`)
- `VariableRefExpression`: Variable references (e.g., `%context`, `%resource`, custom variables)
- `AxisExpression`: Special navigation nodes (e.g., `$this`, scoping operations)

**Position Information:**
The lab supports two approaches for position information:
1. **Character-based**: Use `Position` and `Length` for absolute character positions in the expression
2. **Line/Column-based**: Use `Line`, `Column`, and `Length` for line/column positions

Character-based positioning (`Position`) is preferred as it enables more precise highlighting in the expression editor, and is what is used by the debug-trace entries. When only line/column information is available, the lab will calculate the character position internally.

**Engine Implementation Notes:**
Other engines need to map their internal AST structures to this `JsonNode` format. For reference, the fhirpath.js engine uses this internal structure before conversion:
...
The lab includes conversion logic from the fhirpath.js AST to the standard `JsonNode` format, which demonstrates how engines can transform their native AST representations. Server implementations should perform this conversion server-side rather than relying on client-side transformation for better performance (they can include their internal representation in the response to ease debugging if they choose to do so - this is done in the HAPI engine as their structure is quite different).
