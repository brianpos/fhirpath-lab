...
const host = reactive<Partial<StructureMapTableDefinition> & {
  tableData: StructureMapTableData[]
  loadingData: boolean
  showEmpty: boolean
}>({
  tableData: [],
...
function addNew() { router.push('/StructureMap/:new') }

async function searchPage(url: string) {
  await searchPageFn(host as any, url, (entries) => {
    host.tableData = entries.map<StructureMapTableData>((post) => {
      const vs = post.resource as StructureMap
      return {
        id: vs?.id ?? '',
...
function navigateSelection(data: StructureMapTableData, event?: PointerEvent | MouseEvent) {
  const id = data.id
  if (event && (event as PointerEvent).ctrlKey) {
    window.open('/StructureMap/' + id, '_blank')
  } else {
    router.push('/StructureMap/' + id)
  }
}

function onRowClick(event: PointerEvent, payload: { item: StructureMapTableData }) {
