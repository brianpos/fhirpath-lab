...
import type { StructureDefinition } from 'fhir/r4b'
import {
...
} from '@legacy/models/StructureDefinitionTableData'
import type { ConformanceSearchData } from '@legacy/models/ConformanceSearchData'

useHead({ title: 'Structure Definition' })
...
const host = reactive<Partial<StructureDefinitionTableDefinition> & {
  tableData: StructureDefinitionTableData[]
  loadingData: boolean
  showEmpty: boolean
}>({
  tableData: [],
...
async function searchPage(url: string) {
  await searchPageFn(host as any, url, (entries) => {
    host.tableData = entries.map((post) => {
      ...
      } as StructureDefinitionTableData
    })
  })
}
...
function navigateSelection(data: StructureDefinitionTableData, event?: PointerEvent | MouseEvent) {
  const id = data.id
  if (event && (event as PointerEvent).ctrlKey) {
    window.open('/StructureDefinition/' + id, '_blank')
  } else {
    router.push('/StructureDefinition/' + id)
  }
}

function onRowClick(event: PointerEvent, payload: { item: StructureDefinitionTableData }) {
