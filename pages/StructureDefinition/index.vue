    ...
    <div class="container-fluid bd-layout" style="padding-top: 114px">
      ...
      <v-data-table :headers="columns" :items="tableData"
        ...
        <template v-slot:item.title="{ item }">
          <a :href="`/StructureDefinition/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.title }}</a>
        </template>
        <template v-slot:item.version="{ item }">
          <a :href="`/StructureDefinition/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.version }}</a>
        </template>
        <template v-slot:item.status="{ item }">
          <a :href="`/StructureDefinition/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.status }}</a>
        </template>
        <template v-slot:item.date="{ item }">
          <a :href="`/StructureDefinition/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.date }}</a>
        </template>
        <template v-slot:item.publisher="{ item }">
          <a :href="`/StructureDefinition/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.publisher }}</a>
        </template>
        <template v-slot:item.type="{ item }">
          <a :href="`/StructureDefinition/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.type }}</a>
        </template>
        <template v-slot:item.id="{ item }">
          <a :href="`/StructureDefinition/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.id }}</a>
        </template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async searchPage(url: string) {
      await searchPage(this, url, (entries) => {
        this.tableData = entries.map<StructureDefinitionTableData>((post) => {
          var vs = post.resource as fhir4b.StructureDefinition;
          return {
            id: vs?.id ?? "",
     ...
     *  real <a href>. */
    onCellLinkClick(event: MouseEvent, data: StructureDefinitionTableData) {
