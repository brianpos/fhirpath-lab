    ...
    <div class="container-fluid bd-layout" style="padding-top: 114px">
      ...
      >
        <template v-slot:item.title="{ item }">
          <a :href="`/SubscriptionTopic/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.title }}</a>
        </template>
        <template v-slot:item.version="{ item }">
          <a :href="`/SubscriptionTopic/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.version }}</a>
        </template>
        <template v-slot:item.status="{ item }">
          <a :href="`/SubscriptionTopic/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.status }}</a>
        </template>
        <template v-slot:item.date="{ item }">
          <a :href="`/SubscriptionTopic/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.date }}</a>
        </template>
        <template v-slot:item.publisher="{ item }">
          <a :href="`/SubscriptionTopic/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.publisher }}</a>
        </template>
        <template v-slot:item.base="{ item }">
          <a :href="`/SubscriptionTopic/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.base }}</a>
        </template>
        <template v-slot:item.id="{ item }">
          <a :href="`/SubscriptionTopic/${item.id}`" class="cell-link" @click="onCellLinkClick($event, item)">{{ item.id }}</a>
        </template>
...
export default Vue.extend({
  ...
  methods: {
     ...
     *  real <a href>. */
    onCellLinkClick(event: MouseEvent, data: SubscriptionTopicTableData) {
